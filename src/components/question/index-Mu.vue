    ...
    <el-container style="height: 100%; border: 1px solid #eee">
      <el-container>
        <el-header style="font-size: 20px">
          <el-button type="text" @click="save" >添加选择题试题</el-button>
        </el-header>
...
<script>
  ...
  export default {
    ...
    methods:{
      save:function () {
        this.$router.push("/saveMulti/")
      },
      handleClick:function(questionId){
        this.$router.push("/updateMulti/"+questionId)
      },
      deleteById(questionId) { //删除
        this.$confirm("确定删除这道试题吗？删除后无法恢复",{
          ...
          axios.get("/api/deleteMulti/"+questionId).then(res=>{
            if (res.data=="success"){
              alert("删除成功")
