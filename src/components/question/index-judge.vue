    ...
    <el-container style="height: 100%; border: 1px solid #eee">
      <el-container>
        <el-header style="font-size: 20px">
          <el-button type="text" @click="save" >添加判断题试题</el-button>
        </el-header>
        <el-table
          :data="tableData"
          stripe
...
<script>
  ...
  export default {
    ...
    methods:{
      save:function () {
        this.$router.push("/saveJudge/")
      },
      handleClick:function(questionId){
        this.$router.push("/updateJudge/"+questionId)
      },
      deleteById(questionId) { //删除
        this.$confirm("确定删除这道试题吗？删除后无法恢复",{
          ...
          axios.get("/api/deleteJudge/"+questionId).then(res=>{
            if (res.data=="success"){
              alert("删除成功")
